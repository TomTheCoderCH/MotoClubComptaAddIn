...
- [x] **Gestion des membres et cotisations** — page Membres (sidebar, entre Caisse et Exercices), migration schéma v4 (`members`, `member_dues` + compte 391 Dons) — spec : `docs/superpowers/specs/2026-07-03-members-dues-design.md`, plan : `docs/superpowers/plans/2026-07-03-members-dues.md`
  ...
  - **Historique de cotisations** (`MembreDetailModal`) : case à cocher "payé" toujours éditable pour toute année (`setHistoricalDues` en upsert), y compris les années liées à une écriture comptable existante (`journal_entry_id` non nul) — cocher/décocher ne modifie jamais l'écriture, seulement le suivi ; note de mode de paiement éditable sauf pour les années liées à une écriture (date de paiement affichée en lecture seule à la place) ; contrôle "Ajouter une année" pour créer une ligne sur une année antérieure absente du tableau (validation : 4 chiffres, non future, ≥ 1900, non dupliquée)
